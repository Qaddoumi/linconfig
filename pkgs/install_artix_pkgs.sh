...
# Artix Linux (runit) Packages Installation Script
# This script installs and configures packages for Artix Linux with runit init system
# It uses runit service management instead of systemd
...
# Helper function to enable runit services
enable_runit_service() {
	local service="$1"
	local sv_dir="/etc/runit/sv/$service"
	local runsvdir="/etc/runit/runsvdir/default"
	
	if [[ -d "$sv_dir" ]]; then
		"${ESCALATION_TOOL}" ln -sf "$sv_dir" "$runsvdir/" 2>/dev/null || true
		echo -e "${green}Enabled runit service: $service${no_color}"
	else
		echo -e "${yellow}Runit service not found: $service${no_color}"
	fi
}

# Helper function to start runit services
start_runit_service() {
	local service="$1"
	if [[ -d "/etc/runit/runsvdir/default/$service" ]]; then
		"${ESCALATION_TOOL}" sv start "$service" 2>/dev/null || true
	fi
}

cd ~ || echo -e "${red}Failed to change directory to home${no_color}"

echo -e "${green}\n\n ******************* ARTIX Packages Installation Script ******************* ${no_color}"
...
if [ -n "$is_vm" ]; then
	...
	$ESCALATION_TOOL pacman -S virt-what --noconfirm --needed
...
echo -e "${blue}--------------------------------------------------\n${no_color}"
"${ESCALATION_TOOL}" pacman -S --needed --noconfirm bluetui bluez-utils bluez-runit # Bluetooth TUI with runit service
# Enable bluetooth service for runit
if [[ -d /etc/runit/sv/bluetoothd ]]; then
	"${ESCALATION_TOOL}" ln -sf /etc/runit/sv/bluetoothd /etc/runit/runsvdir/default/ 2>/dev/null || true
	"${ESCALATION_TOOL}" sv start bluetoothd 2>/dev/null || true
fi
echo -e "${blue}--------------------------------------------------\n${no_color}"
...
echo -e "${blue}--------------------------------------------------\n${no_color}"
# udisks2 on Artix uses dbus activation, no separate runit service needed
# Just ensure dbus is running (should be enabled from base install)
...
echo -e "${blue}--------------------------------------------------\n${no_color}"
"${ESCALATION_TOOL}" pacman -S --needed --noconfirm tlp tlp-runit # TLP for power management with runit service
# Enable TLP service for runit
if [[ -d /etc/runit/sv/tlp ]]; then
	"${ESCALATION_TOOL}" ln -sf /etc/runit/sv/tlp /etc/runit/runsvdir/default/ 2>/dev/null || true
fi
echo -e "${blue}--------------------------------------------------\n${no_color}"
"${ESCALATION_TOOL}" pacman -S --needed --noconfirm lm_sensors # Hardware monitoring
echo -e "${blue}--------------------------------------------------\n${no_color}"
"${ESCALATION_TOOL}" pacman -S --needed --noconfirm thermald thermald-runit # Intel thermal daemon with runit service
# Enable thermald service for runit
enable_runit_service "thermald"
echo -e "${blue}--------------------------------------------------\n${no_color}"
...
echo -e "${blue}--------------------------------------------------\n${no_color}"
"${ESCALATION_TOOL}" pacman -S --needed --noconfirm libvirt libvirt-runit # Libvirt for managing virtualization with runit service
echo -e "${blue}--------------------------------------------------\n${no_color}"
...
echo -e "${green}Enabling and starting libvirtd service (runit)${no_color}"
# Enable libvirtd service for runit
if [[ -d /etc/runit/sv/libvirtd ]]; then
	"${ESCALATION_TOOL}" ln -sf /etc/runit/sv/libvirtd /etc/runit/runsvdir/default/ 2>/dev/null || true
	"${ESCALATION_TOOL}" sv start libvirtd 2>/dev/null || true
fi
if [[ -d /etc/runit/sv/virtlogd ]]; then
	"${ESCALATION_TOOL}" ln -sf /etc/runit/sv/virtlogd /etc/runit/runsvdir/default/ 2>/dev/null || true
	"${ESCALATION_TOOL}" sv start virtlogd 2>/dev/null || true
fi
...
echo -e "${green}Installing QEMU Guest Agents and enabling their services (runit)${no_color}"
...
# Enable qemu-guest-agent service for runit
if [[ -d /etc/runit/sv/qemu-guest-agent ]]; then
	"${ESCALATION_TOOL}" ln -sf /etc/runit/sv/qemu-guest-agent /etc/runit/runsvdir/default/ 2>/dev/null || true
	"${ESCALATION_TOOL}" sv start qemu-guest-agent 2>/dev/null || true
fi

# Enable spice-vdagentd service for runit
if [[ -d /etc/runit/sv/spice-vdagentd ]]; then
	"${ESCALATION_TOOL}" ln -sf /etc/runit/sv/spice-vdagentd /etc/runit/runsvdir/default/ 2>/dev/null || true
	"${ESCALATION_TOOL}" sv start spice-vdagentd 2>/dev/null || true
fi
...
if [ "$is_vm" = true ]; then
	...
	echo -e "${green}Restarting libvirtd service to apply changes (runit)...${no_color}"
	"${ESCALATION_TOOL}" sv restart libvirtd 2>/dev/null || true
...
"${ESCALATION_TOOL}" pacman -S --needed --noconfirm sddm sddm-runit || true
# Disable any existing display manager and enable sddm for runit
# Remove any existing display manager symlinks
"${ESCALATION_TOOL}" rm -f /etc/runit/runsvdir/default/gdm 2>/dev/null || true
"${ESCALATION_TOOL}" rm -f /etc/runit/runsvdir/default/lightdm 2>/dev/null || true
"${ESCALATION_TOOL}" rm -f /etc/runit/runsvdir/default/lxdm 2>/dev/null || true
"${ESCALATION_TOOL}" rm -f /etc/runit/runsvdir/default/ly 2>/dev/null || true
# Enable sddm service for runit
if [[ -d /etc/runit/sv/sddm ]]; then
	"${ESCALATION_TOOL}" ln -sf /etc/runit/sv/sddm /etc/runit/runsvdir/default/ 2>/dev/null || true
fi
