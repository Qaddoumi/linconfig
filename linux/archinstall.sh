...
if systemd-detect-virt --vm &>/dev/null; then
    ...
    case "$VIRT_TYPE" in
        "kvm"|"qemu"|"microsoft")
            ...
            if [[ -e "/dev/virtio-ports/com.redhat.spice.0" ]] || [[ -e "/dev/virtio-ports/org.spice-space.webdav.0" ]] || lspci | grep -qi "qxl"; then
...
su "$USER_NAME" <<USEREOF
    ...
    bash <(curl -sL https://raw.githubusercontent.com/Qaddoumi/linconfig/quickshell/pkgs/install.sh) --is-vm "$isVM" || echo "Failed to run the install script"
USEREOF
...
POSTINSTALLEOF
    ...
    info "bash <(curl -sL https://raw.githubusercontent.com/Qaddoumi/linconfig/quickshell/pkgs/install.sh) --is-vm \"$IS_VM\""
fi
